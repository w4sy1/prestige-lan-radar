...
def normalize(rows):
    ...
    result=[];seen=set()
    for row in rows:
        ip=str(ipaddress.ip_address(row['ip']));mac=str(row['mac']).lower().replace('-',':')
        if not re.fullmatch(r'(?:[0-9a-f]{2}:){5}[0-9a-f]{2}',mac) or mac in ('00:00:00:00:00:00','ff:ff:ff:ff:ff:ff'):raise ValueError('Nieprawidłowy MAC.')
        if mac in seen:raise ValueError('Powtórzony MAC; wybierz pojedynczy adres urządzenia.')
        seen.add(mac);result.append({'ip':ip,'mac':mac,'hostname':str(row.get('hostname',''))[:253],'vendor':str(row.get('vendor',''))[:200]})
    return result
...
def neighbors():
    ...
    dedup={r['mac'].lower():r for r in data}
    return normalize(list(dedup.values()))
    ...
    with connect(path) as db:
        ...
        for row in rows:
            ...
            if not old:changes.append('NEW_DEVICE')
                ...
                if old['ip']!=row['ip']:changes.append('IP_CHANGE')
            ...
            db.execute('INSERT INTO devices VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(mac) DO UPDATE SET ip=excluded.ip,hostname=CASE WHEN excluded.hostname="" THEN devices.hostname ELSE excluded.hostname END,vendor=CASE WHEN excluded.vendor="" THEN devices.vendor ELSE excluded.vendor END,last_seen=excluded.last_seen,status=excluded.status',
                       (row['mac'],row['ip'],row['hostname'],row['vendor'],stamp,stamp,'online','Nieznane'))
